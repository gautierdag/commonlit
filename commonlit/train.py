...
for ws in [0.1, 0.25]:
    for (pretrain, pretrain_external_files) in [
        (False, False),
        (True, False),
        (True, True),
    ]:
        for multitask in [False, True]:
            if ws == 0.1 and not pretrain and not multitask:
                continue

            hyperparameter_defaults["warmup_steps"] = ws
            hyperparameter_defaults["pretrain"] = pretrain
            hyperparameter_defaults["pretrain_external_files"] = pretrain_external_files
            hyperparameter_defaults["multitask"] = multitask

            # Log Group
            group = (
                f"{hyperparameter_defaults['bert_model']}_{ws}"
                + wandb.util.generate_id()
            )
            if pretrain:
                if pretrain_external_files:
                    group += "_pretrain_ext"
                else:
                    group += "_pretrain"
            if multitask:
            ...
            if hyperparameter_defaults["pretrain"]:
                ...
                pretrain_dataset, pretrain_val_dataset = get_mlm_dataset(
                    tokenizer,
                    use_external_files=hyperparameter_defaults[
                        "pretrain_external_files"
                    ],
                )
            ...
            for fold, (train_ids, val_ids) in enumerate(skf.split(dataset)):
                ...
                if hyperparameter_defaults["multitask"]:
                    ...
                    print(
                        f"Best multitask score for fold {fold}: {best_multitask_score}"
                    )
